...
import { workflowSteps } from '@/data/Pages/home/4-workflow';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          {workflowSteps.map((step, idx) => (
            ...
            >
              <div className={`p-8 rounded-xl bg-black/20 border border-white/5 hover:border-white/10 transition-colors h-full flex flex-col items-start text-left group `}>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-6 font-bold border border-white/5 group-hover:scale-110 transition-transform duration-300`}>
                  {step.id}
                </div>
                <h3 className="text-2xl font-semibold text-white mb-2 text-{step.color}">
                  <span className={`text-${step.color}`}>{step.title}</span>
                </h3>
